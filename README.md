...
Next.js + TypeScript + Tailwind CSS + Supabase로 만드는 익명 고백/위로 커뮤니티 스타터입니다.
...
지금 상태는 바로 실행 가능한 UI 스캐폴드이며, 데이터 계층은 `mock*` 데이터로 동작합니다. 기본 기획은 `고해성사`, `위로받고 싶어요`, `해결책을 제시해주세요` 3개 게시판과 슬로건 `bless you`를 중심으로 잡혀 있습니다.
