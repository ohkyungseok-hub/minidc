...
-- Safe to re-run: titles are unique enough to avoid duplicates.
-- ============================================================
do $$
declare
  v_author_id     uuid;
...
begin
  ...
  insert into public.posts (
    board_id, author_id, title, content,
    is_notice, is_anonymous, up_count, down_count, view_count,
  ...
  from (values
    ...
    -- ── 고해성사 (confession) ──────────────────────────────
    (
      v_confession_id, v_author_id,
      '10년 전 친구에게 거짓말을 한 게 아직도 걸려요',
      ...
      false, true, 31, 1, 240,
      now() - interval '6 days', null::timestamptz
    ),
    (
      v_confession_id, v_author_id,
      '부모님한테 용돈을 받으면서 취직했다고 거짓말했습니다',
    ...
    (
      v_confession_id, v_author_id,
      '친구의 남자친구가 다른 여자와 카페에 있는 걸 봤어요',
      '분명히 아는 사이 같았고, 꽤 친밀해 보였습니다. 친구한테 말해야 할지 말아야 할지 2주째 고민 중입니다. 말했다가 괜한 사이가 될까봐 무서워요.',
      false, true, 44, 3, 412,
      now() - interval '4 days', null::timestamptz
    ),
    (
      v_confession_id, v_author_id,
      '사실 저는 주변 사람들이 다 부럽습니다',
    ...
    (
      ...
      false, true, 72, 0, 643,
      now() - interval '1 day', null::timestamptz
    ),

    -- ── 해결책을 제시해주세요 (solutions) ─────────────────
    (
      v_solutions_id, v_author_id,
      '부모님 노후 자금 문제, 어떻게 대화를 꺼내야 할까요',
      ...
      false, false, 29, 1, 274,
      now() - interval '4 days', null::timestamptz
    ),
    (
      v_solutions_id, v_author_id,
      '직장 내 괴롭힘인지 아닌지 판단이 안 돼요',
