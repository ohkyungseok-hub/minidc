-- ============================================================
-- Seed 017: 10 additional sample posts
-- Spread across confession / comfort / solutions boards.
-- ============================================================
do $$
declare
  v_author_id     uuid;
...
begin
  ...
  if v_author_id is null then
    raise exception 'No users found. Create at least one user first.';
  end if;
  ...
  if v_confession_id is null or v_comfort_id is null or v_solutions_id is null then
    raise exception 'Required boards missing. Run 007 or 014 migration first.';
  end if;
  ...
  select v_confession_id, v_author_id, '10년 전 친구에게 거짓말을 한 게 아직도 걸려요',
  ...
  where not exists (select 1 from public.posts where title = '10년 전 친구에게 거짓말을 한 게 아직도 걸려요');

  insert into public.posts (board_id, author_id, title, content, is_notice, is_anonymous, up_count, down_count, view_count, created_at)
  select v_confession_id, v_author_id, '부모님한테 용돈을 받으면서 취직했다고 거짓말했습니다',
    '백수인 걸 들키기 싫어서 취직했다고 했습니다. 벌써 8개월째 이 거짓말을 유지하고 있는데, 이제는 너무 지쳤습니다. 어떻게 털어놓아야 할지 모르겠어요.',
    false, true, 18, 2, 185, now() - interval '5 days'
  where not exists (select 1 from public.posts where title = '부모님한테 용돈을 받으면서 취직했다고 거짓말했습니다');

  insert into public.posts (board_id, author_id, title, content, is_notice, is_anonymous, up_count, down_count, view_count, created_at)
  select v_confession_id, v_author_id, '친구의 남자친구가 다른 여자와 카페에 있는 걸 봤어요',
    '분명히 아는 사이 같았고, 꽤 친밀해 보였습니다. 친구한테 말해야 할지 말아야 할지 2주째 고민 중입니다. 말했다가 괜한 사이가 될까봐 무서워요.',
    false, true, 44, 3, 412, now() - interval '4 days'
  where not exists (select 1 from public.posts where title = '친구의 남자친구가 다른 여자와 카페에 있는 걸 봤어요');

  insert into public.posts (board_id, author_id, title, content, is_notice, is_anonymous, up_count, down_count, view_count, created_at)
  select v_confession_id, v_author_id, '사실 저는 주변 사람들이 다 부럽습니다',
  ...
  where not exists (select 1 from public.posts where title = '사실 저는 주변 사람들이 다 부럽습니다');

  -- ── 위로받고 싶어요 (comfort) ──────────────────────────
  insert into public.posts (board_id, author_id, title, content, is_notice, is_anonymous, up_count, down_count, view_count, created_at)
  select v_comfort_id, v_author_id, '퇴사하고 나서 아무것도 하기가 싫어요',
  ...
  where not exists (select 1 from public.posts where title = '퇴사하고 나서 아무것도 하기가 싫어요');

  insert into public.posts (board_id, author_id, title, content, is_notice, is_anonymous, up_count, down_count, view_count, created_at)
  select v_comfort_id, v_author_id, '30대가 됐는데 아직도 뭘 좋아하는지 모르겠어요',
  ...
  where not exists (select 1 from public.posts where title = '30대가 됐는데 아직도 뭘 좋아하는지 모르겠어요');

  insert into public.posts (board_id, author_id, title, content, is_notice, is_anonymous, up_count, down_count, view_count, created_at)
  select v_comfort_id, v_author_id, '가장 친한 친구가 저를 무시하는 것 같아요',
  ...
  where not exists (select 1 from public.posts where title = '가장 친한 친구가 저를 무시하는 것 같아요');

  insert into public.posts (board_id, author_id, title, content, is_notice, is_anonymous, up_count, down_count, view_count, created_at)
  select v_comfort_id, v_author_id, '매일 아침 일어나는 게 무서워요',
  ...
  where not exists (select 1 from public.posts where title = '매일 아침 일어나는 게 무서워요');

  -- ── 해결책을 제시해주세요 (solutions) ─────────────────
  insert into public.posts (board_id, author_id, title, content, is_notice, is_anonymous, up_count, down_count, view_count, created_at)
  select v_solutions_id, v_author_id, '부모님 노후 자금 문제, 어떻게 대화를 꺼내야 할까요',
  ...
  where not exists (select 1 from public.posts where title = '부모님 노후 자금 문제, 어떻게 대화를 꺼내야 할까요');

  insert into public.posts (board_id, author_id, title, content, is_notice, is_anonymous, up_count, down_count, view_count, created_at)
  select v_solutions_id, v_author_id, '직장 내 괴롭힘인지 아닌지 판단이 안 돼요',
  ...
  where not exists (select 1 from public.posts where title = '직장 내 괴롭힘인지 아닌지 판단이 안 돼요');
